...
def access_gsheet():
    ...
    wks1 = gc.open('test').get_worksheet(0)
    wks1_data = wks1.get_all_records()
    wks1 = pd.DataFrame(wks1_data)
    wks2 = gc.open('test').get_worksheet(1)
    wks2_data = wks2.get_all_records()
    wks2 = pd.DataFrame(wks2_data)
    wks3 = gc.open('test').get_worksheet(2)
    wks3_data = wks3.get_all_records()
    wks3 = pd.DataFrame(wks3_data)
    
    wks = wks1.merge(wks2, how='outer',on=list(wks2.columns))
    wks = wks.merge(wks3, how='outer',on=list(wks3.columns))
    return(wks)     
